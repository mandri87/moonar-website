      ...
      <Container>
        ...
        <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {referenceSections.map((section, index) => (
            <Reveal key={section.slug} delay={index * 80} className="h-full">
              <Link
                href={`/references/${section.slug}`}
                className="card-elevated flex h-full flex-col gap-3 rounded-md border border-border bg-surface p-6"
              >
